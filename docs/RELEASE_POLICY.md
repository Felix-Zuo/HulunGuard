...
4. Run tests, validation, calibration, calibration drift review, threat model check, agent compatibility, integration kit verification, onboarding verification, adapter matrix, collector smoke, batched ingestion and stdin payload smoke, trace doctor, schema compatibility, and retention cleanup dry-run.
