...
1. Local gates pass: Ruff, Bandit, compileall, pytest, validation, calibration, calibration drift, threat model check, agent compatibility, integration kit verification, onboarding verification, adapter matrix, collector smoke, managed collector smoke, collector operations status, collector service-template generation, trace doctor, schema compatibility, retention cleanup dry-run, scan benchmark, real-world benchmark, and build.
