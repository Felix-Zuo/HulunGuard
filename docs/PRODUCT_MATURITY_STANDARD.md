...
The current target is M3 hardening. The privacy-safe redaction baseline, adapter SDK, MCP stdio server, local HTTP collector baseline, managed collector flush/scan baseline, graceful shutdown verification baseline, collector operations status baseline, grouped collector health diagnostics baseline, collector Prometheus metrics baseline, collector Prometheus alert-rule baseline, cross-platform collector service-template baseline, collector service-lifecycle baseline, OpenTelemetry/OpenInference import/export baseline, hosted-platform fixture baseline, OpenAI Agents SDK native trace adapter baseline, durable async/batched ingestion baseline, in-memory/stdin runtime payload baseline, LangSmith native service export baseline, release provenance baseline, installed release-verification baseline, 100-trajectory labeled calibration baseline, checked-in calibration drift baseline, adapter conformance baseline, adapter integration matrix baseline, expanded public-safe real-world benchmark baseline, agent compatibility matrix baseline, verified integration kit baseline, zero-knowledge onboarding baseline, trace-doctor diagnostic baseline, retention cleanup baseline, schema compatibility baseline, and threat-model baseline are implemented. The next gate is expanding native service export coverage and connector operations maturity across more hosted observability surfaces.
