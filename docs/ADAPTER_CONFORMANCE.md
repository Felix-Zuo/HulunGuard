...
| collector-smoke-tested | OTLP/HTTP JSON, generic HTTP adapter payload path, managed flush/scan, Prometheus metrics, alert-rule export | `python -m hulun_guard collector smoke --json`, `python -m hulun_guard collector smoke --managed --scan --init-if-missing --json`, `python -m hulun_guard collector metrics --require-status-file`, and `python -m hulun_guard collector alert-rules --force --json` |
